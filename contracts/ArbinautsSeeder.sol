...
contract ArbinautsSeeder is IArbinautsSeeder {
    /**
     * @notice Generate a pseudo-random Arbinaut seed using the previous blockhash and arbinaut ID.
     */
    // prettier-ignore
    function generateSeed(uint256 arbinautId, IArbinautsDescriptor descriptor) external view override returns (Seed memory) {
        uint256 pseudorandomness = uint256(
            keccak256(abi.encodePacked(blockhash(block.number - 1), arbinautId))
        );
