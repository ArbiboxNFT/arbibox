...
contract ArbinautsNFTVault is ReentrancyGuard, Ownable, IERC721Receiver, IERC1155Receiver {
    using SafeERC20 for IERC20;

    // The ERC721 token contracts
    INFT public ARBINAUTS;
    ...
    modifier holdsBoxu() {
        require(!mustHold || ARBINAUTS.balanceOf(msg.sender) > 0, "Must hold a Arbinaut NFT");
        _;
    }

    modifier onlyArbinautsTreasury() {
        require(msg.sender == arbinautsTreasury || msg.sender == owner(), "Sender not allowed");
        _;
    }
    ...
    function setArbinauts(address _arbinauts) external onlyOwner {
        ARBINAUTS = INFT(_arbinauts);
    }

    function setBoxu(address _boxu) external onlyOwner {
        DUST = IBoxu(_boxu);
    }
    ...
    function updateArbinautsTreasury(address _treasury) external onlyOwner {
        arbinautsTreasury = _treasury;
    }
    ...
    function createAuction(address _nftContract, uint256 _tokenId, bool _is1155, uint32 _startTime, uint32 _endTime, uint256 _startingPrice, string calldata _slug) external onlyArbinautsTreasury nonReentrant {
    ...
    function createManyAuctionSameProject(address _nftContract, uint256[] calldata _tokenIds, bool _is1155, uint32 _startTime, uint32 _endTime, uint256 _startingPrice, string calldata _slug) external onlyArbinautsTreasury nonReentrant {
    ...
    function createBlindAuction(address _nftContract, bool _is1155, uint32 _startTime, uint32 _endTime, string calldata _hiddenImage, uint256 _startingPrice, string calldata _slug) external onlyArbinautsTreasury nonReentrant {
        uint32 id = currentAuctionId++;
    ...
    function createManyBlindAuctionSameProject(address _nftContract, bool _is1155, uint16 _numAuctions, uint32 _startTime, uint32 _endTime, string calldata _hiddenImage, uint256 _startingPrice, string calldata _slug) external onlyArbinautsTreasury nonReentrant {
    ...
    function updateBlindAuction(uint32 _id, uint256 _tokenId) external onlyArbinautsTreasury {
    ...
    function updateManyBlindAuction(uint32[] calldata _ids, uint256[] calldata _tokenIds) external onlyArbinautsTreasury {
    ...
    function updateBlindAuction1155(uint32 _id, bool _is1155) external onlyArbinautsTreasury {
        auctionId[_id].is1155 = _is1155;
    }

    function updateManyBlindAuction1155(uint32[] calldata _ids, bool _is1155) external onlyArbinautsTreasury {
        for(uint i = 0; i < _ids.length; i++) {
            auctionId[_ids[i]].is1155 = _is1155;
        }
    }

    function updateBlindImage(uint32 _id, string calldata _hiddenImage) external onlyArbinautsTreasury {
        auctionId[_id].hiddenImage = _hiddenImage;
    }

    function updateManyBlindImage(uint32[] calldata _ids, string calldata _hiddenImage) external onlyArbinautsTreasury {
        for(uint i = 0; i < _ids.length; i++) {
            auctionId[_ids[i]].hiddenImage = _hiddenImage;
        } 
    }

    function updateOpenseaSlug(uint32 _id, string calldata _slug) external onlyArbinautsTreasury {
        auctionId[_id].openseaSlug = _slug;
    }

    function updateManyOpenseaSlug(uint32[] calldata _ids, string calldata _slug) external onlyArbinautsTreasury {
        for(uint i = 0; i < _ids.length; i++) {
            auctionId[_ids[i]].openseaSlug = _slug;
        } 
    }

    function updateAuctionStartingPrice(uint32 _id, uint256 _startingPrice) external onlyArbinautsTreasury {
    ...
    function updateManyAuctionStartingPrice(uint32[] calldata _ids, uint256 _startingPrice) external onlyArbinautsTreasury {
        for(uint i = 0; i < _ids.length; i++) {
    ...
    function updateAuctionEndTime(uint32 _id, uint32 _newEndTime) external onlyArbinautsTreasury {
    ...
    function updateManyAuctionEndTime(uint32[] calldata _ids, uint32 _newEndTime) external onlyArbinautsTreasury {
        for(uint i = 0; i < _ids.length; i++) {
    ...
    function emergencyCancelAllAuctions() external onlyArbinautsTreasury {
    ...
    function emergencyCancelAuction(uint32 _id) external onlyArbinautsTreasury {
    ...
    function emergencyRescueNFT(address _nft, uint256 _tokenId, bool _is1155) external onlyArbinautsTreasury {
        if(_is1155) {
    ...
    function emergencyRescueERC20(IERC20 token, uint256 amount, address to) external onlyArbinautsTreasury {
     ...
     */
    function createBid(uint32 _id, uint256 _bidAmount) external payable holdsBoxu nonReentrant {
