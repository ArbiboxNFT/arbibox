...
contract BoxPeerExchange is ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;

    // The Metabox ERC721 token contracts
    INFT public ARBIBOX;
    ...
    modifier holdsNFTLister() {
        require(allListersAllowed || ARBIBOX.balanceOf(msg.sender) > 0, "Must hold a Metadust NFT");
        _;
    }

    modifier holdsNFTBuyer() {
        require(allBuyersAllowed || ARBIBOX.balanceOf(msg.sender) > 0, "Must hold a Metadust NFT");
        _;
    }

    modifier holdsNFTOfferer() {
        require(allOfferersAllowed || ARBIBOX.balanceOf(msg.sender) > 0, "Must hold a Metadust NFT");
        _;
    }
    ...
    function createListing(uint256 _dustAmount, uint256 _requestedEth, uint32 _endTime) external notPaused holdsNFTLister nonReentrant {
        ...
        BOX.safeTransferFrom(msg.sender, address(this), _dustAmount);
    ...
    function cancelListing(uint32 _id) external nonReentrant {
        ...
        activeListingCount--;
        BOX.safeTransfer(listingId[_id].lister, listingId[_id].dustAmount);
    ...
    /**
     * @notice Create a offer for BOX, with a given ETH amount.
    ...
    function acceptOffer(uint32 _id) external nonReentrant {
        ...
        BOX.safeTransfer(listingId[_id].offerer, listingId[_id].dustAmount);
    ...
    function buyNow(uint32 _id) external payable holdsNFTBuyer nonReentrant {
        ...
        BOX.safeTransfer(msg.sender, listingId[_id].dustAmount);
        _safeTransferETHWithFallback(taxWallet, taxAmount);
        _safeTransferETHWithFallback(listingId[_id].lister, finalEthAmount);
