...
contract BoxuPeerExchange is ReentrancyGuard, Ownable {
    using SafeERC20 for IERC20;

    // The Metaboxu ERC721 token contracts
    INFT public ARBINAUTS;

    // The address of the WETH contract
    address public WETH;

    // The address of the BOXU contract
    IERC20 public BOXU;
    ...
    modifier holdsNFTLister() {
        require(allListersAllowed || ARBINAUTS.balanceOf(msg.sender) > 0, "Must hold a Metaboxu NFT");
        _;
    }

    modifier holdsNFTBuyer() {
        require(allBuyersAllowed || ARBINAUTS.balanceOf(msg.sender) > 0, "Must hold a Metaboxu NFT");
        _;
    }

    modifier holdsNFTOfferer() {
        require(allOfferersAllowed || ARBINAUTS.balanceOf(msg.sender) > 0, "Must hold a Metaboxu NFT");
        _;
    }
    ...
    event ListingCreated(uint256 indexed ListingId, uint256 startTime, uint256 endTime, uint256 BoxuAmount, uint256 EthPrice);
    ...
    event ListingRefunded(uint256 indexed ListingId, address Lister, uint256 BoxuRefundAmount, address Offerer, uint256 OffererRefundAmount, address Caller);
    event ListingCanceled(uint256 indexed ListingId, address Lister, uint256 BoxuAmount, uint256 TimeStamp);
    ...
    function createListing(uint256 _boxuAmount, uint256 _requestedEth, uint32 _endTime) external notPaused holdsNFTLister nonReentrant {
        require((_requestedEth * 10**9) / _boxuAmount >= minListPrice(), "Listing Price too low");
        uint32 startTime = uint32(block.timestamp);
        uint32 _listingId = currentId++;

        listingId[_listingId].lister = payable(msg.sender);
        listingId[_listingId].boxuAmount = _boxuAmount;
        ...
        BOXU.safeTransferFrom(msg.sender, address(this), _boxuAmount);

        emit ListingCreated(_listingId, startTime, _endTime, _boxuAmount, _requestedEth);
    }
    ...
    function cancelListing(uint32 _id) external nonReentrant {
        ...
        activeListingCount--;
        BOXU.safeTransfer(listingId[_id].lister, listingId[_id].boxuAmount);

        emit ListingCanceled(_id, listingId[_id].lister, listingId[_id].boxuAmount, block.timestamp);
        emit ListingRefunded(_id, listingId[_id].lister, listingId[_id].boxuAmount, listingId[_id].offerer, listingId[_id].currentOffer, msg.sender);
    }
    ...
    function editListingPrice(uint32 _id, uint256 _requestedEth) external notPaused nonReentrant {
        ...
        require(listingStatus(_id) == 1, "Listing is not active");
        require((_requestedEth * 10**9) / listingId[_id].boxuAmount >= minListPrice(), "Listing Price too low");
    ...
    /**
     * @notice Create a offer for BOXU, with a given ETH amount.
     * @dev This contract only accepts payment in ETH.
     */
    function createOffer(uint32 _id) external payable holdsNFTOfferer nonReentrant {
        require(listingStatus(_id) == 1, 'Boxu Listing is not Active');
    ...
    function acceptOffer(uint32 _id) external nonReentrant {
        ...
        BOXU.safeTransfer(listingId[_id].offerer, listingId[_id].boxuAmount);
    ...
    function buyNow(uint32 _id) external payable holdsNFTBuyer nonReentrant {
        ...
        BOXU.safeTransfer(msg.sender, listingId[_id].boxuAmount);
        _safeTransferETHWithFallback(taxWallet, taxAmount);
        _safeTransferETHWithFallback(listingId[_id].lister, finalEthAmount);
